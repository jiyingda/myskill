...
export async function dirSize(dir) {
  ...
  async function walk(d) {
    ...
    for (const e of entries) {
      ...
      // follow symlinks: use stat (not lstat) to determine true type
      const st = await statSafe(p);
      if (!st) continue;
...
export async function copyDirRecursive(src, dst) {
  ...
  for (const e of entries) {
    ...
    // follow symlinks via stat
    const st = await statSafe(s);
    if (!st) continue;
    if (st.isDirectory()) await copyDirRecursive(s, d);
...
export function assertSafeId(id) {
  // Allow '/' for nested skill paths like "category/skill-name"
  // but disallow empty segments, '..' traversal, or leading dots
  const segments = id ? id.split('/') : [];
  if (
    segments.length === 0 ||
    segments.some(s => !s || s === '..' || s.startsWith('.') || s.includes('\\'))
  ) {
